...
The following URL-encoded query parameters are required:
...
<RequestExample>
    ```http Example Request Structure (URL Encoded)
    GET /api/v1/addresses/verify
        ?name=Kohler+Group
        &email=parisian.kristoffer%40yahoo.com
        &phone=%2B12069536978
...
<ResponseExample>
    ```text Example Response Structure (Status Code: 200)
    No Content
    ```
